...
# 换成自己的mysql服务器信息
conn = DBConn(host='127.0.0.1', user='user', passwd='password', database='test')
print('链接创建成功')
...
print('insert: insert count:%d; ids:%s' % (len(insert_rows), ','.join([str(row.get('id')) for row in insert_rows])))

update_rows = conn.update('t_jobs', [{'id': 1, 'gender': 'male', 'class': '1'},
                              {'id': 2, 'class': '2'},
                              {'name': 'cici', 'gender': 'female'}
                              ])
print('update: update count:%d; ids:%s' % (len(update_rows), ','.join([str(row.get('id')) for row in update_rows])))
